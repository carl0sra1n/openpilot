...
class Uploader:
  ...
  def list_upload_files(self, metered: bool) -> Iterator[tuple[str, str, str]]:
    ...
    sentry_mode = self.params.get_bool("SentryMode")
    for logdir in listdir_by_creation(self.root):
      ...
      for name in sorted(names, key=lambda n: self.immediate_priority.get(n, 1000)):
        ...
        if metered:
          ...
          if sentry_mode and name.endswith(".hevc"):
            # allow all cameras if sentry mode is enabled
            pass
          elif name not in ["qlog", "qlog.zst", "qcamera.ts"] and not any(logdir.startswith(r.split('|')[-1]) for r in requested_routes):
            continue
