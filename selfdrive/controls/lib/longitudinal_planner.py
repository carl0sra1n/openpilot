...
A_CRUISE_MAX_VALS = [1.6, 1.2, 0.7, 0.6]  # Reduced from 0.8 at 25 m/s
A_CRUISE_MAX_BP = [0.0, 10.0, 25.0, 40.0]
CONTROL_N_T_IDX = ModelConstants.T_IDXS[:CONTROL_N]
ALLOW_THROTTLE_THRESHOLD = 0.4
MIN_ALLOW_THROTTLE_SPEED = 2.5

# Lookup table for turns - disabled turn speed limiting
_A_TOTAL_MAX_V = [9.9, 9.9]  # Set high to effectively disable
...
class LongitudinalPlanner(LongitudinalPlannerSP):
  ...
  def update(self, sm):
    ...
    # Float 5mph over v_cruise - allow coasting above cruise speed
    if v_ego > (v_cruise + 0.5):
      v_cruise = min(v_ego - 0.5, v_cruise + 2.2352)  # 2.2352 m/s = 5 mph
